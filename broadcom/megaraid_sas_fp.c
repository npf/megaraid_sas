...
static u8 MR_PdInterfaceTypeGet(u32 pd, MR_DRV_RAID_MAP_ALL *map)
{
...
static int MR_PopulateDrvRaidMap(struct megasas_instance *instance, u64 map_id)
{
	...
	if (instance->maxRaidMapSize) {
		...
		desc_table = (MR_RAID_MAP_DESC_TABLE *)((void *)fw_map_dyn + le32_to_cpu(fw_map_dyn->descTableOffset));
		...
		ld_count = le16_to_cpu(fw_map_dyn->ldCount);
		pDrvRaidMap->ldCount = (__le16)cpu_to_le16(ld_count);
		pDrvRaidMap->fpPdIoTimeoutSec = fw_map_dyn->fpPdIoTimeoutSec;
		pDrvRaidMap->totalSize = cpu_to_le32(sizeof(MR_DRV_RAID_MAP_ALL));
			...
			switch (le32_to_cpu(desc_table->raidMapDescType)) {
				...
				case RAID_MAP_DESC_TYPE_TGTID_INFO:
					...
					for(j = 0; j < le32_to_cpu(desc_table->raidMapDescElements); j++)
						pDrvRaidMap->ldTgtIdToLd[j] = le16_to_cpu(fw_map_dyn->ldTgtIdToLd[j]);
					break;
				case RAID_MAP_DESC_TYPE_ARRAY_INFO:
					fw_map_dyn->arMapInfo = (MR_ARRAY_INFO *) (raid_map_data +
						le32_to_cpu(desc_table->raidMapDescOffset));
				
					memcpy(pDrvRaidMap->arMapInfo, fw_map_dyn->arMapInfo, 
						sizeof(MR_ARRAY_INFO) * le32_to_cpu(desc_table->raidMapDescElements));
					break;
				case RAID_MAP_DESC_TYPE_SPAN_INFO:
					fw_map_dyn->ldSpanMap = (MR_LD_SPAN_MAP *) (raid_map_data +
									le32_to_cpu(desc_table->raidMapDescOffset));
		...
		fw_map_ext = ( MR_FW_RAID_MAP_EXT *) fusion->ld_map[(map_id & 1)];
		ld_count = le16_to_cpu(fw_map_ext->ldCount);
		...
		fw_map_old = (MR_FW_RAID_MAP_ALL*) fusion->ld_map[(map_id & 1)];
...
u8 MR_ValidateMapInfo(struct megasas_instance *instance, u64 map_id)
{
	...
	BUG_ON(!pDrvRaidMap);
	if (instance->maxRaidMapSize)
		expected_size = sizeof(MR_DRV_RAID_MAP_ALL);
	else if(instance->supportmax256vd) 
	...
	if (le32_to_cpu(pDrvRaidMap->totalSize) != expected_size) {
		printk(KERN_ERR "megasas: map info structure size 0x%x is not matching expected size 0x%x\n",
		   ...
		   RAID_CONTEXT *pRAID_Context, MR_DRV_RAID_MAP_ALL *map)
{
	...
	u8      *pPdInterface = &io_info->pdInterface;
	u32	rowMod, armQ, arm;
	u64	logArm;
	struct fusion_context *fusion;
	...
	*pDevHandle = MR_DEVHANDLE_INVALID; // set dev handle as invalid.
	...
	if (pd != MR_PD_INVALID) {
		...
		*pPdInterface = MR_PdInterfaceTypeGet(pd, map);
		/* Make sure r1_alt_dev_handle is set in io_info, only if it is R1 FP Write */
		if ((instance->adapter_type == VENTURA_SERIES) && (raid->level == 1) && !io_info->isRead) {
...
static u8 megasas_get_best_arm_pd(struct megasas_instance *instance, PLD_LOAD_BALANCE_INFO lbInfo, 
		struct IO_REQUEST_INFO *io_info, MR_DRV_RAID_MAP_ALL *drv_map)
{
	MR_LD_RAID  *raid;
	u16	pd1_devHandle;
	u16     pend0, pend1, ld;
