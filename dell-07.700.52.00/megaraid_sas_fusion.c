...
static int megasas_create_sg_sense_fusion(struct megasas_instance *instance)
{
	...
	/* create sense buffer for the raid 1/10 fp and rmw raid 5/6 */
		...
		bool is_32bit)
{
	...
	unsigned long flags;
	if (is_32bit)
		writel(le32_to_cpu(req_desc->u.low), &(regs)->inbound_single_queue_port);
	/* dude to hardware bug in A0 ventura, we can not do 64 bit writes, we can remove this once it is fixed*/
	else if (instance->is_ventura) {
		spin_lock_irqsave(&instance->hba_lock, flags);
		writel(le32_to_cpu(req_desc->u.low), &(regs)->inbound_low_queue_port);
		writel(le32_to_cpu(req_desc->u.high), &(regs)->inbound_high_queue_port);
		mmiowb();
...
megasas_fusion_update_can_queue(struct megasas_instance *instance, int fw_boot_context)
{
	...
	if (!instance->is_ventura) // ventura FW does not fill outbound_scratch_pad_3 with queue depth
	...
	if (fw_boot_context == OCR_CONTEXT) {
		 ...
		 * MSIx vectors.
		 */
		if ((!instance->is_rdpq) && (!instance->is_ventura))
		...
		u8 status, u8 ext_status, u32 data_length, u8 *sense)
{
	...
	case MFI_STAT_SCSI_DONE_WITH_ERROR:
		...
		/*
 		 ...
 		 */
		scsi_set_resid(scmd, scsi_bufflen(scmd) - data_length);
		break;
			  ...
			  struct megasas_cmd_fusion *cmd)
{
	...
	if (instance->is_ventura) {
		...
		if ((raid->level == 0) || (raid->level == 1)) {
			...
			if (fp_possible && !io_info.isRead &&
				raid->capability.fpCacheBypassCapable) {
				pRAID_Context->raid_context_g35.routingFlags.bits.sld = 1;
...
/* CS2.1 */	
		...
		if (instance->is_ventura) {
			if (io_info.isRead) {
				...
				if (io_request->RaidContext.raid_context_g35.streamDetected && 
					(raid->level == 5) && (raid->writeMode == MR_RL_WRITE_THROUGH_MODE)){
					if(pRAID_Context->raid_context_g35.routingFlags.bits.cpuSel == MR_RAID_CTX_CPUSEL_FCFS)
						pRAID_Context->raid_context_g35.routingFlags.bits.cpuSel = MR_RAID_CTX_CPUSEL_0;
				}
			}
		}
	} /* Not FP */
...
void megasas_fpio_to_ldio(struct megasas_instance *instance, struct megasas_cmd_fusion *cmd, struct scsi_cmnd *scmd)
{
	struct fusion_context *fusion;
	RAID_CONTEXT_UNION *pRAID_Context;
	MR_LD_RAID *raid;
	...
	cmd->io_request->IoFlags &= cpu_to_le16(~MPI25_SAS_DEVICE0_FLAGS_ENABLED_FAST_PATH);

	/* if the numSGE > max_sge_in_main_sge set the chain offset*/
	if (cmd->io_request->RaidContext.raid_context_g35.numSGE > fusion->max_sge_in_main_msg)
		cmd->io_request->ChainOffset = fusion->chain_offset_io_request;
				   ...
				   struct scsi_cmnd *scmd)
{
	...
	if (r1_cmd)
		megasas_fire_cmd_fusion(instance, r1_cmd->request_desc, instance->is_ventura);
...
complete_cmd_fusion(struct megasas_instance *instance, u32 MSIxIndex)
{
	...
	while ((d_val.u.low != UINT_MAX) && (d_val.u.high != UINT_MAX))
	{
		...
		case MPI2_FUNCTION_SCSI_IO_REQUEST :  /*Fast Path IO.*/
			 ...
			 */
			if (!cmd_fusion->isRaid_1_fp_write && fusion->load_balance_info &&
		...
		case MEGASAS_MPI2_FUNCTION_LD_IO_REQUEST : /* LD-IO Path */
 			...
 			*/	if (r1_cmd &&  r1_cmd->isRaid_1_fp_write
				 ...
				 * makes sure that if any one of command fails, return fail status to scsi layer
				 */	
...
/*
 * megasas_return_shutdown_cmd :	Re-fire management commands
 * @instance:				Controller's soft instance
*/
void megasas_return_shutdown_cmd(struct megasas_instance *instance)
{
	int j;
	...
	for (j = instance->max_scsi_cmds ; j < instance->max_fw_cmds; j++) {
		...
		if (cmd_mfi->frame->dcmd.opcode == MR_DCMD_CTRL_SHUTDOWN) {
...
int megasas_reset_fusion(struct Scsi_Host *shost, u8 reason)
{
	...
	if (megasas_wait_for_outstanding_fusion(instance, reason,
		...
		megasas_return_shutdown_cmd(instance);
