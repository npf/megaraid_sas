...
static inline int megasas_alloc_ctrl_dma_buffers(struct megasas_instance *instance)
{
	...
	if (!instance->system_info_buf)
		dev_err(&instance->pdev->dev, "Failed to allocate system info buffer\n");

	if (!instance->pd_info)
		dev_err(&instance->pdev->dev, "Failed to allocate pd_info buffer\n");
...
megasas_aen_polling(struct work_struct *work)
{
	...
	if (!instance) {
	    dev_err(&instance->pdev->dev, "invalid instance!\n");
		kfree(ev);
		return;
	}
