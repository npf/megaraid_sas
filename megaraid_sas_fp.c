...
static int MR_PopulateDrvRaidMap(struct megasas_instance *instance)
{
	...
	if (instance->maxRaidMapSize) {
		...
		dev_err(&instance->pdev->dev,
			"drv map %p ldCount %d\n", drv_map, le16_to_cpu(fw_map_dyn->ldCount));
#endif
		...
		ld_count = (u16)le32_to_cpu(fw_map_dyn->ldCount);
			...
			switch (le32_to_cpu(desc_table->raidMapDescType)) {
				...
				case RAID_MAP_DESC_TYPE_SPAN_INFO:
					...
					for (j = 0; j < ld_count; j++) {
					        printk("megaraid_sas(%d) : fw_map_dyn->ldSpanMap[%d].ldRaid.targetId 0x%x "
						...
						&fw_map_dyn->ldSpanMap[j].ldRaid, &pDrvRaidMap->ldSpanMap[j].ldRaid);
					}
#endif
				break;
				default:
		...
		ld_count = (u16)le32_to_cpu(fw_map_ext->ldCount);
...
#if VD_EXT_DEBUG
		for (i = 0; i < ld_count; i++) {
	        printk("megaraid_sas(%d) : fw_map_ext->ldSpanMap[%d].ldRaid.targetId 0x%x "
		   ...
		   RAID_CONTEXT *pRAID_Context, MR_DRV_RAID_MAP_ALL *map)
{
	...
	if (pd != MR_PD_INVALID) {
		*pDevHandle = MR_PdDevHandleGet(pd, map);
		...
		if (instance->is_ventura && (raid->level == 1)) {
