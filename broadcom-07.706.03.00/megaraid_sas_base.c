...
module_param(event_log_level, int, S_IRUGO | S_IWUSR);
...
static
int megasas_alloc_ctrl_mem(struct megasas_instance *instance);
static
inline void megasas_free_ctrl_mem(struct megasas_instance *instance);
static
inline int megasas_alloc_ctrl_dma_buffers(struct megasas_instance *instance);
static
inline void megasas_free_ctrl_dma_buffers(struct megasas_instance *instance);
static
inline void megasas_init_ctrl_params(struct megasas_instance *instance);
void
	...
	struct megasas_dcmd_frame *dcmd, dma_addr_t dma_addr, u32 dma_len)
{
	if (instance->consistent_mask_64bit) {
		...
		dcmd->sgl.sge32[0].phys_addr = cpu_to_le32(lower_32_bits(dma_addr));
...
megasas_decode_evt(struct megasas_instance *instance)
{
	...
	if ((event_log_level < MFI_EVT_CLASS_DEBUG) ||
		...
		printk("megarid_sas: provided event log level is out of range, setting"
...
#endif
{
	...
	/* Don't send SYNCHRONIZE_CACHE to VDs, if firmware can not handle the same*/
	if (MEGASAS_IS_LOGICAL(scmd->device) && (scmd->cmnd[0] == SYNCHRONIZE_CACHE)
...
/*
* megasas_set_dynamic_target_properties - Update sdev structure based on controller's FW capabilities
*
...
* Returns void
*/
void megasas_set_dynamic_target_properties(struct scsi_device *sdev, bool is_target_prop)
{
	u16 pd_index = 0, ld;
	u32 device_id;
	struct megasas_instance *instance;
...
/*
 * This function will set few parameters required for NVMe JBODs and VDs
 */
static inline void megasas_set_nvme_device_properties(struct scsi_device *sdev, u32 max_io_size)
{
	struct megasas_instance *instance;
    u32 mr_nvme_pg_size;
...
#ifdef KERNEL_SUPPORT_BLK_QUEUE_VIRT_BOUNDARY_API
    blk_queue_virt_boundary(sdev->request_queue, mr_nvme_pg_size - 1);
	sdev_printk(KERN_INFO, sdev, "BRCM: QUEUE_FLAG_SG_GAPS in SG set from %s %d fw page size %d\n",
			__func__, __LINE__, mr_nvme_pg_size);
#else
 ...
 * Returns void
 */
static void megasas_set_static_target_properties(struct scsi_device *sdev, bool is_target_prop)
{
	u16	target_index = 0;
	u8 interface_type;
	...
	*/
	...
	if (is_target_prop) {
		...
		if (tgt_device_qd && (tgt_device_qd <= instance->host->can_queue))
			device_qd = tgt_device_qd;

		/* max_io_size_kb will be set to non zero for NVMe based VDs and sysPDs */
	...
	/* scsi.mq performance better with device qd set as controller's can_queue */
		...
		for (i = 0; i < instance->max_fw_cmds; i++) {
			...
			if (cmd_fusion->sync_cmd_idx != (u32)ULONG_MAX) {
				...
				if (cmd_mfi->sync_cmd &&
					...
					cmd_mfi->frame->hdr.cmd_status = MFI_STAT_WRONG_STATE;
					 ...
					 int initial)
{
	...
	if ((instance->adapter_type != MFI_SERIES) && !instance->mask_interrupts)
...
static int megasas_reset_bus_host(struct scsi_cmnd *scmd)
{
	...
	scmd_printk(KERN_INFO, scmd, "Controller reset is requested due to IO timeout\n"
		"SCSI command pointer: (%p)\t SCSI host state: %d\t SCSI host busy: %d\t FW outstanding: %d\n",
		scmd, scmd->device->host->shost_state, atomic_read((atomic_t *)&scmd->device->host->host_busy),
		atomic_read(&instance->fw_outstanding));
	...
	if (instance->adapter_type == MFI_SERIES)
		...
		if (cmd)
			megasas_dump_frame(cmd->io_request, sizeof(MEGASAS_RAID_SCSI_IO_REQUEST));
...
static ssize_t
megasas_fw_cmds_outstanding_show(struct device *cdev, struct device_attribute *attr,
        char *buf)
{
        struct Scsi_Host *shost = class_to_shost(cdev);
        struct megasas_instance *instance = (struct megasas_instance *)shost->hostdata;
...
static ssize_t
megasas_sgl_type_io_stats_show(struct device *cdev, struct device_attribute *attr,
	char *buf)
{
	struct Scsi_Host *shost = class_to_shost(cdev);
	struct megasas_instance *instance = (struct megasas_instance *)shost->hostdata;

	return snprintf(buf, PAGE_SIZE, "IEEE SGL IOs: %d\t PRP SGL IOs: %d\t R1 FP writes count: %d\t"
		" SGE holes: %d/%d/%d\n", atomic_read(&instance->ieee_sgl), atomic_read(&instance->prp_sgl),
		atomic_read(&instance->r1_fp_writes_count), atomic_read(&instance->sge_holes_type1),
		atomic_read(&instance->sge_holes_type2), atomic_read(&instance->sge_holes_type3));
}
...
static DEVICE_ATTR(io_stats, S_IRUGO,
		megasas_sgl_type_io_stats_show, NULL);
...
static int megasas_create_frame_pool(struct megasas_instance *instance)
{
	...
	instance->frame_dma_pool = pci_pool_create("mfi_sg",
					instance->pdev, instance->mfi_frame_size, 256, 0);
...
megasas_get_pd_info(struct megasas_instance *instance, struct scsi_device *sdev)
{
	...
	megasas_set_dma_settings(instance, dcmd, instance->pd_info_h, sizeof(struct MR_PD_INFO));
...
megasas_get_bios_data(struct megasas_instance *instance)
{
	...
	megasas_set_dma_settings(instance, dcmd, ci_h, sizeof(struct MR_BIOS_DATA));
	...
	*/
	...
	switch(ret)
	{
		case DCMD_SUCCESS:
				...
				le16_to_cpus((u16 *)&ci->adapterOperations4);
				...
				instance->support_morethan256jbod = 
					ci->adapterOperations4.supportPdMapTargetId;
				instance->support_nvme_passthru =
					ci->adapterOperations4.supportNVMePassthru;
				instance->task_abort_tmo = ci->TaskAbortTO;
				instance->max_reset_tmo =
					ci->MaxResetTO;
				instance->divert_io_with_chain_frame =
					ci->adapterOperations4.DivertIOWithChainFrame;
				...
				dev_info(&instance->pdev->dev, "jbod sync map legacy/extended\t: %s/%s\n",
						instance->use_seqnum_jbod_fp ? "Yes" : "No",
						instance->support_morethan256jbod ? "Yes" : "No");
				...
				dev_info(&instance->pdev->dev, "FW support divert IO with chain frame: %d\n",
					 instance->divert_io_with_chain_frame);
...
static int megasas_check_cpx_support( struct megasas_instance *instance)
{
	...
	megasas_set_dma_settings(instance, dcmd, cpx_init_data_h,
...
#else
//	if (instance->msix_vectors) TBD:assess it later whether it's really required
...
static int megasas_init_fw(struct megasas_instance *instance)
{
	u32 max_sectors_1;
	u32 max_sectors_2;
	u32 tmp_sectors, msix_enable, scratch_pad_2, scratch_pad_3, scratch_pad_4;
	struct megasas_register_set __iomem *reg_set;
	...
	/* Find first memory bar */
		...
		if (pci_request_selected_regions(instance->pdev, 1<<instance->bar, "megasas: LSI")) {
	...
	if (msix_enable && !msix_disable) {
		...
		if (fusion) {
				if (instance->adapter_type == THUNDERBOLT_SERIES) { /* Thunderbolt family*/
	...
	if (instance->adapter_type == VENTURA_SERIES) {
		scratch_pad_4 = readl(&instance->reg_set->outbound_scratch_pad_4);
		if ((scratch_pad_4 & MR_NVME_PAGE_SIZE_MASK) >= MR_DEFAULT_NVME_PAGE_SHIFT)
			instance->nvme_page_size = 1 << (scratch_pad_4 & MR_NVME_PAGE_SIZE_MASK);
	...
	if ((resetwaittime < 1) || (resetwaittime > MEGASAS_RESET_WAIT_TIME))
...
/* Input:   dcmd.opcode         - MR_DCMD_DEV_GET_TARGET_PROP
*          dcmd.mbox.b[0]      - DCMD is to be fired for LD or system PD. 0 = system PD, 1 = LD
*          dcmd.mbox.s[1]      - TargetID for LD/system PD
*          dcmd.sge IN         - Pointer to return MR_TARGET_DEV_PROPERTIES
* Desc:    This DCMD will fetch few properties of LD/system PD defined in MR_TARGET_DEV_PROPERTIES
*      eg. Queue Depth, MDTS value.
*      Currently DCMD send by drivers whenever new target is added to the OS.
* Status:  MFI_STAT_OK: SUCCESS
*          MFI_STAT_DEVICE_NOT_FOUND: Invalid targetID for system PD/LD.
*/
int
megasas_get_target_prop(struct megasas_instance *instance, struct scsi_device *sdev)
{
	...
	dcmd->timeout = 0;
	dcmd->pad_0 = 0;
	dcmd->data_xfer_len = cpu_to_le32(sizeof(struct MR_TARGET_PROPERTIES));
	...
	if (ret != DCMD_SUCCESS)
		dev_err(&instance->pdev->dev, "return from %s %d return value %d\n",
 ...
 */
static int megasas_set_dma_mask(struct megasas_instance *instance)
{
	...
	if (IS_DMA64) {
		...
		if ((*pdev->dev.dma_mask == DMA_BIT_MASK(64)) &&
			...
			megasas_dma_set_mask_and_coherent(&pdev->dev, DMA_BIT_MASK(32)))) {
			/*
		 	* If 32 bit DMA mask fails, then
		 	* try for 64 bit mask for FW capable of handling 64 bit DMA.
		 	*/
			scratch_pad_2 = readl
				(&instance->reg_set->outbound_scratch_pad_2);
			...
			if (!(scratch_pad_2 & MR_CAN_HANDLE_64_BIT_DMA_OFFSET))
					goto fail_set_dma_mask;
			else if (megasas_dma_set_mask_and_coherent(&pdev->dev, DMA_BIT_MASK(64)))
					goto fail_set_dma_mask;
		}
 ...
 */
static inline bool
megasas_irq_set_affinity_hint(unsigned int msix_vector,
...
try_io_apic:
	...
	for (i = 0; i < instance->msix_vectors; i++) {
		...
		if (request_irq(megasas_get_irq(instance, i),
			...
			for (j = 0; j < i; j++) {
				megasas_irq_set_affinity_hint(megasas_get_irq(instance, j),
					NULL);
		...
		if (!megasas_irq_set_affinity_hint(megasas_get_irq(instance, i),
				get_cpu_mask(cpu)))
			dev_err(&instance->pdev->dev,
...
/*
 * megasas_destroy_irqs-                Unregister IRQs and IRQ affinity
 * @instance:                           Adapter soft state
 * return:                              void
 */
static void
megasas_destroy_irqs(struct megasas_instance *instance)
{
	...
	if (instance->msix_vectors) {
		for (i = 0; i < instance->msix_vectors; i++) {
			megasas_irq_set_affinity_hint(megasas_get_irq(instance, i),
				NULL);
...
static inline int megasas_alloc_mfi_ctrl_mem(struct megasas_instance *instance)
{
	...
	if (!instance->producer || !instance->consumer) {
		dev_err(&instance->pdev->dev, "Failed to allocate memory for "
		"producer, consumer\n");
		return -1;
	}
...
/**
 * megasas_set_adapter_type - 	 Set adapter type. Supported controllers can be divided in
...
{
	/* Except PERC5 controller, rest controllers has vendor ID- 0x1000/PCI_VENDOR_ID_LSI_LOGIC */
...
/**
 * megasas_alloc_ctrl_mem - 	 Allocate per controller memory for core data structures which
 * 				 are not common accross MFI adapters and fusion adapters.
 * 				 For MFI based adapters, allocate producer and consumer buffers.
 * 				 For fusion adapters, allocate memory for fusion context.
 * @instance:			 Adapter soft state
 * return:			 0 for SUCCESS
 */ 
...
static inline void megasas_free_ctrl_mem(struct megasas_instance *instance)
{
	if (instance->adapter_type == MFI_SERIES) {
		if (instance->producer)
			pci_free_consistent(instance->pdev, sizeof(u32), instance->producer,
			    instance->producer_h);
		if (instance->consumer)
			pci_free_consistent(instance->pdev, sizeof(u32), instance->consumer,
			    instance->consumer_h);
	} else
...
/*
 * megasas_alloc_ctrl_dma_buffers-	Allocate consistent DMA buffers during driver load time
 *
 * @instance-				Adapter soft instance
 * @return-				O for SUCCESS
 */
static inline int megasas_alloc_ctrl_dma_buffers(struct megasas_instance *instance)
{
	...
	if (!instance->evt_detail) {
		dev_err(&instance->pdev->dev, "Failed to allocate event detail buffer\n");
		return -ENOMEM;
	}

	if (fusion) {
		fusion->ioc_init_request = dma_alloc_coherent(&pdev->dev,
					sizeof(MPI2_IOC_INIT_REQUEST),
					&fusion->ioc_init_request_phys, GFP_KERNEL);
	...
	instance->pd_list_buf = pci_alloc_consistent(pdev,
				MEGASAS_MAX_PD * sizeof(struct MR_PD_LIST),
	...
	instance->ctrl_info_buf = pci_alloc_consistent(pdev,
					sizeof(struct megasas_ctrl_info),
	...
	instance->ld_list_buf = pci_alloc_consistent(pdev,
					sizeof(struct MR_LD_LIST),
	...
	instance->ld_targetid_list_buf = pci_alloc_consistent(pdev,
						sizeof(struct MR_LD_TARGETID_LIST),
						&instance->ld_targetid_list_buf_h);

	if (!instance->ld_targetid_list_buf) {
		dev_err(&pdev->dev, "Failed to allocate LD targetid list buffer\n");
		return -ENOMEM;
	}
...
/*
 * megasas_free_ctrl_dma_buffers-		Free consistent DMA buffers allocated during driver load time
 *
 * @instance-				Adapter soft instance
 *
 */
static inline void megasas_free_ctrl_dma_buffers(struct megasas_instance *instance)
{
	struct pci_dev *pdev = instance->pdev;
	struct fusion_context *fusion = instance->ctrl_context;
	...
	if (fusion && fusion->ioc_init_request)
		dma_free_coherent(&pdev->dev, sizeof(MPI2_IOC_INIT_REQUEST),
			fusion->ioc_init_request, fusion->ioc_init_request_phys);

	if (instance->pd_list_buf)
		pci_free_consistent(pdev, MEGASAS_MAX_PD * sizeof(struct MR_PD_LIST),
			instance->pd_list_buf, instance->pd_list_buf_h);

	if (instance->ld_list_buf)
		pci_free_consistent(pdev, sizeof(struct MR_LD_LIST),
			instance->ld_list_buf, instance->ld_list_buf_h);

	if (instance->ld_targetid_list_buf)
		pci_free_consistent(pdev, sizeof(struct MR_LD_TARGETID_LIST),
			instance->ld_targetid_list_buf, instance->ld_targetid_list_buf_h);

	if (instance->ctrl_info_buf)
		pci_free_consistent(pdev, sizeof(struct megasas_ctrl_info),
			instance->ctrl_info_buf, instance->ctrl_info_buf_h);
...
/*
 * megasas_init_ctrl_params-		Initialize controller's instance parameters before FW init
 * @instance-				Adapter soft instance
 * @return-				void
 */
static inline void megasas_init_ctrl_params(struct megasas_instance *instance)
{
	...
	atomic_set(&instance->fw_outstanding, 0);
	atomic_set(&instance->ieee_sgl, 0);
	atomic_set(&instance->prp_sgl, 0);
...
inline static int megasas_wait_for_adapter_operational(struct megasas_instance *instance)
{
	...
	if (adp_state != MEGASAS_HBA_OPERATIONAL) {
		dev_info(&instance->pdev->dev,
			 "%s timed out while waiting for HBA to recover, adp_state %d\n",
			 __func__, adp_state);
		return 1;
	}
...
/**
 * megasas_pci_device_is_present -	Check whether PCI device is present for not
 * @pdev:		                    PCI device structure
 * return:                          true: if device is present physically
 *                                  false: if device is removed physically
 */
bool megasas_pci_device_is_present(struct pci_dev *pdev)
{
        u32 v;
	    if (pci_bus_read_config_dword(pdev->bus, pdev->devfn, PCI_VENDOR_ID, &v))
		    return false;
...
#endif
{
	...
	if (!megasas_pci_device_is_present(pdev))
		      ...
		      struct megasas_iocpacket *ioc)
{
	...
	if ((ioc->frame.hdr.cmd >= MFI_CMD_OP_COUNT) ||
		...
		!instance->support_nvme_passthru)) {
		printk(KERN_ERR "megasas: Received invalid ioctl command 0x%x\n",
			ioc->frame.hdr.cmd);
		return -ENOTSUPP;
	}
	...
	if (opcode == MR_DCMD_CTRL_IO_METRICS_GET) {
	...
	if (ioc->sense_len) {
		...
		sense_ptr = (unsigned long *) ((unsigned long)ioc->frame.raw +
				ioc->sense_off);
		if (copy_to_user((void __user *)((unsigned long) get_unaligned((unsigned long *) sense_ptr)),
				 sense, ioc->sense_len)) {
			printk(KERN_ERR "megasas: Failed to copy out to user "
					"sense data\n");
			error = -EFAULT;
...
static ssize_t
megasas_sysfs_show_support_nvme_encapsulation(struct device_driver *dd, char *buf)
{
	return sprintf(buf, "%u\n", support_nvme_encapsulation);
}

static DRIVER_ATTR(support_nvme_encapsulation, S_IRUGO,
	megasas_sysfs_show_support_nvme_encapsulation, NULL);
