...
u16 get_updated_dev_handle(struct megasas_instance *instance, PLD_LOAD_BALANCE_INFO lbInfo, 
				struct IO_REQUEST_INFO *in_info, MR_DRV_RAID_MAP_ALL *drv_map);
 ...
 * return-					   true : allocation does not cross same 4GB boundary
 *						       false: allocation crosses same 4GB boundary
 */
	...
	(struct megasas_instance *instance, dma_addr_t start_addr, size_t size)
{
	...
	if (upper_32_bits(start_addr) != upper_32_bits(end_addr)) {
		dev_err(&instance->pdev->dev, "Failed to get same 4GB boundary:"
				" start_addr: 0x%llx end_addr: 0x%llx\n",
						  ...
						  *instance, u32 blk_tag)
{
	...
	if (blk_tag > instance->max_mpt_cmds) {
		dev_err(&instance->pdev->dev,
			"request tags (%04d) max_mpt_cmds (%04d) "
			"can_queue (%04d) max_scsi_cmds (%04d)\n",
			blk_tag, instance->max_mpt_cmds,
...
megasas_free_cmds_fusion(struct megasas_instance *instance)
{
	...
	if (fusion->sense)
		pci_pool_free(fusion->sense_dma_pool, fusion->sense,
...
static int megasas_create_sg_sense_fusion(struct megasas_instance *instance)
{
	...
	fusion->sg_dma_pool = 
			pci_pool_create("mr_sg", instance->pdev,
				instance->max_chain_frame_sz, 4096, 0);
	...
	fusion->sense = pci_pool_alloc(fusion->sense_dma_pool,
	...
	if (!megasas_check_same_4gb_region(instance,
		fusion->sense_phys_addr, sense_sz)) {
		pci_pool_free(fusion->sense_dma_pool, fusion->sense,
			      fusion->sense_phys_addr);
		fusion->sense = NULL;
		pci_pool_destroy(fusion->sense_dma_pool);
		...
		fusion->sense_dma_pool =
			pci_pool_create("mr_sense_align", instance->pdev,
...
megasas_alloc_cmdlist_fusion(struct megasas_instance *instance)
{
	 ...
	 * commands.
	 */
	fusion->cmd_list = kmalloc(sizeof(struct megasas_cmd_fusion *) * max_mpt_cmd,
						GFP_KERNEL);
	if (!fusion->cmd_list) {
		dev_err(&instance->pdev->dev,
			"Failed from %s %d\n",  __func__, __LINE__);
...
retry_alloc:
	...
	fusion->io_request_frames =
			pci_pool_alloc(fusion->io_request_frames_pool ,
				(GFP_KERNEL & (~__GFP_WAIT)), &fusion->io_request_frames_phys);
#else
	fusion->io_request_frames =
			pci_pool_alloc(fusion->io_request_frames_pool ,
	...
	if (!fusion->io_request_frames) {
		...
		if (instance->max_fw_cmds >= MEGASAS_REDUCE_QD_COUNT) {
			pci_pool_destroy(fusion->io_request_frames_pool);
	...
	if (!megasas_check_same_4gb_region(instance,
		...
		fusion->io_request_frames_pool =
				pci_pool_create("mr_ioreq_align",
						instance->pdev,
						fusion->io_frames_alloc_sz,
...
megasas_alloc_reply_fusion(struct megasas_instance *instance)
{
	...
	if (!megasas_check_same_4gb_region(instance,
		...
		fusion->reply_frames_desc_pool =
				pci_pool_create("mr_reply_align",
						instance->pdev,
						fusion->reply_alloc_sz * count,
...
megasas_alloc_rdpq_fusion(struct megasas_instance *instance)
{
	...
	struct fusion_context *fusion;
	pMpi2ReplyDescriptorsUnion_t reply_desc;
	Mpi2ReplyDescriptorsUnion_t *rdpq_chunk_virt[RDPQ_MAX_CHUNK_COUNT];
	...
	array_size = sizeof(Mpi2IOCInitRDPQArrayEntry) * MAX_MSIX_QUEUES_FUSION;
	...
	fusion->reply_frames_desc_pool_align =
			pci_pool_create("mr_rdpq_align", instance->pdev,
					chunk_size,
					roundup_pow_of_two(chunk_size), 0);
 ...
 * memory for reply queues according. Driver uses limitation of VENTURA_SERIES to
 * manage INVADER_SERIES as well.
 */
	...
	for (i = 0; i < dma_alloc_count; i++) {
		 ...
		 * Alignment will be used such a way that next allocation if success, will
		 * always meet same 4gb region requirement.
		 * rdpq_tracker keep track of each buffer's physical, virtual address
		 * and pci pool descriptor. It will help driver while freeing the resources.
		 *
		 */
		if (!megasas_check_same_4gb_region(instance,
			rdpq_chunk_phys[i], chunk_size)) {
			pci_pool_free(fusion->reply_frames_desc_pool,
				      rdpq_chunk_virt[i],
				      rdpq_chunk_phys[i]);

			rdpq_chunk_virt[i] =
				pci_pool_alloc(fusion->reply_frames_desc_pool_align,
					       GFP_KERNEL, &rdpq_chunk_phys[i]);
			if (!rdpq_chunk_virt[i]) {
				dev_err(&instance->pdev->dev,
					"Failed from %s %d\n",  __func__, __LINE__);
	...
	for (k = 0; k < dma_alloc_count; k++) {
		for (i = 0; i < RDPQ_MAX_INDEX_IN_ONE_CHUNK; i++) {
			...
			fusion->reply_frames_desc[abs_index] =
					(Mpi2ReplyDescriptorsUnion_t*)((u8 *)rdpq_chunk_virt[k] + offset);
...
megasas_free_rdpq_fusion(struct megasas_instance *instance) {
	...
	for (i = 0; i < RDPQ_MAX_CHUNK_COUNT; i++) {
		if (fusion->rdpq_tracker[i].pool_entry_virt)
			pci_pool_free(fusion->rdpq_tracker[i].dma_pool_ptr,
	...
	if (fusion->reply_frames_desc_pool_align)
		pci_pool_destroy(fusion->reply_frames_desc_pool_align);
...
megasas_alloc_cmds_fusion(struct megasas_instance *instance)
{
	...
	for (i = 0; i < instance->max_mpt_cmds; i++) {
		...
		cmd->index = i + 1; 
		cmd->scmd = NULL;
		cmd->sync_cmd_idx = (i >= instance->max_scsi_cmds && i < instance->max_fw_cmds) ?
...
megasas_fusion_update_can_queue(struct megasas_instance *instance, int fw_boot_context)
{
	...
	if (instance->adapter_type < VENTURA_SERIES) // ventura FW does not fill outbound_scratch_pad_3 with queue depth
...
megasas_ioc_init_fusion(struct megasas_instance *instance)
{
	...
	IOCInitMessage->TimeStamp = cpu_to_le64((tv.tv_sec * 1000) + (tv.tv_usec / 1000));
	...
	if (instance->consistent_mask_64bit == true)
	...
	init_frame->queue_info_new_phys_addr_lo = cpu_to_le32(lower_32_bits(ioc_init_handle));
	init_frame->queue_info_new_phys_addr_hi = cpu_to_le32(upper_32_bits(ioc_init_handle));
...
fail_fw_init:
	dev_err(&instance->pdev->dev, "IOC INIT command return status %s for"
...
static inline int megasas_allocate_raid_maps(struct megasas_instance *instance)
{
	...
	for (i = 0; i < 2; i++) {
		...
		fusion->ld_drv_map[i] = (void *) __get_free_pages(__GFP_ZERO | GFP_KERNEL,
			fusion->drv_map_pages);

		if (!fusion->ld_drv_map[i]) {
			fusion->ld_drv_map[i] = vmalloc(fusion->drv_map_sz);

			if (!fusion->ld_drv_map[i]) {
				dev_err(&instance->pdev->dev, "Could not allocate memory for local map "
					" size requested: %d", fusion->drv_map_sz);
				...
				memset(fusion->ld_drv_map[i], 0, fusion->drv_map_sz);
		}
	}
...
/**
 * megasas_configure_queue_sizes -     Calculate size of request desc queue, reply desc queue,
...
void megasas_configure_queue_sizes(struct megasas_instance *instance)
{
	...
	fusion->request_alloc_sz = sizeof(MEGASAS_REQUEST_DESCRIPTOR_UNION) * instance->max_mpt_cmds;
 ...
 * @instance:		Adapter soft state
 */
static void inline megasas_free_ioc_init_cmd(struct megasas_instance *instance)
{
	struct fusion_context *fusion;
	...
	if (fusion->ioc_init_cmd && fusion->ioc_init_cmd->frame)
		dma_free_coherent(&instance->pdev->dev,
			IOC_INIT_FRAME_SIZE, fusion->ioc_init_cmd->frame,
...
megasas_init_adapter_fusion(struct megasas_instance *instance)
{
	...
	dev_warn(&instance->pdev->dev, "Max Chain Size %d \n", instance->max_chain_frame_sz);

	fusion->max_sge_in_main_msg =
		(MEGA_MPI2_RAID_DEFAULT_IO_FRAME_SIZE
			- offsetof(MEGASAS_RAID_SCSI_IO_REQUEST, SGL))/16;

	fusion->max_sge_in_chain = instance->max_chain_frame_sz /
...
void
map_cmd_status(struct fusion_context *fusion, struct scsi_cmnd *scmd,
		u8 status, u8 ext_status, u32 data_length, u8 *sense)
{
	u8 cmd_type;
	int resid;
	...
	case MFI_STAT_SCSI_DONE_WITH_ERROR:
		...
		if (ext_status == SAM_STAT_CHECK_CONDITION) {
			...
			memcpy(scmd->sense_buffer, sense, SCSI_SENSE_BUFFERSIZE);
		...
		if (resid &&
			((cmd_type == READ_WRITE_LDIO) || (cmd_type == READ_WRITE_SYSPDIO)))
...
megasas_is_prp_possible(struct megasas_instance *instance, struct scsi_cmnd *scmd, int sge_count)
{
	...
	mr_nvme_pg_size = max_t(u32, instance->nvme_page_size, MR_DEFAULT_NVME_PAGE_SIZE);
	fusion = instance->ctrl_context;
...
/*
 * This code detects gaps/holes in IO data buffers.
 * What does holes/gaps mean?
 * Any SGE except first one in a SGL starts at non NVME page size aligned address OR
 * Any SGE except last one in a SGL ends at non NVME page size boundary.
 *
 * Driver has already informed OS by setting boundary rules for bio merging done at
 * NVME page size boundary by calling kernel API blk_queue_virt_boundary inside
 * slave_config but still there is possibility of IO coming with holes to driver
 * because of IO merging done by IO scheduler.
 *
 * With SCSI BLK MQ enabled, there will be no IO with holes as there is no IO scheduling
 * so no IO merging.
 *
 * With SCSI BLK MQ disabled, IO scheduler may merge IOs and then sending IO holes.
 * Though driver can request block layer to disable IO merging by calling-
 * queue_flag_set_unlocked(QUEUE_FLAG_NOMERGES, sdev->request_queue) but user may
 * tune sysfs parameter- nomerges again to 0 causing IO merging hence IO with holes.
 *
 * If in future IO scheduling is enabled with SCSI BLK MQ, this algorithm to detect holes
 * will be required in driver to for SCSI BLK MQ enabled case as well.
 *
 */
	scsi_for_each_sg(scmd, sg_scmd, sge_count, i) {
		...
		if ((sge_count > 1) && (i == 0)) {
			if ((mega_mod64((sg_dma_address(sg_scmd) + sg_dma_len(sg_scmd)), mr_nvme_pg_size))) {
				build_prp = false;
		...
		if ((sge_count > 1) && (i == (sge_count - 1))) {
			if (mega_mod64(sg_dma_address(sg_scmd), mr_nvme_pg_size)) {
				build_prp = false;
...
/**
 * megasas_make_prp_nvme - Prepare PRPs(Physical Region Page)- SGLs specific to NVMe drives only
 * @instance:				Adapter soft state
...
megasas_make_prp_nvme(struct megasas_instance *instance, struct scsi_cmnd *scmd,
	pMpi25IeeeSgeChain64_t sgl_ptr, struct megasas_cmd_fusion *cmd, int sge_count)
{
	int sge_len, offset, num_prp_in_chain = 0;
	pMpi25IeeeSgeChain64_t main_chain_element, ptr_first_sgl;
	u64 *ptr_sgl;
	...
	main_chain_element = (pMpi25IeeeSgeChain64_t)
	    ((u8 *)sgl_ptr + sizeof(MPI25_IEEE_SGE_CHAIN64));
	...
	for (;;) {
		...
		if (unlikely(!page_mask_result)) {
			scmd_printk(KERN_NOTICE, scmd, "BRCM: Put prp pointer as we are at page boundary"
					" ptr_sgl: 0x%p\n", ptr_sgl);
			ptr_sgl_phys += 8;
	...
	main_chain_element->Length = cpu_to_le32(num_prp_in_chain * sizeof(u64));
		...
		if ((sg_processed ==  (fusion->max_sge_in_main_msg - 1)) &&
			...
			if (instance->adapter_type >= INVADER_SERIES) {
				if (!(le16_to_cpu(cmd->io_request->IoFlags) &
				      ...
				      cmd->io_request->RaidContext.raid_context.divertedFPIO)
 ...
 * @sge_count		scatter gather entry count
 *
 * This function will build SGLs based on device type.
 * For NVMe drives, there is different way of building SGLs in NVMe's native
 * format- PRPs(Physical Region Page).
 */
static
void megasas_make_sgl(struct megasas_instance *instance, struct scsi_cmnd *scp,
			struct megasas_cmd_fusion *cmd, int sge_count)
{
	...
	if ((le16_to_cpu(cmd->io_request->IoFlags) &
		...
		build_prp = megasas_make_prp_nvme(instance, scp,
			(pMpi25IeeeSgeChain64_t) &cmd->io_request->SGL, cmd, sge_count);
				...
				struct IO_REQUEST_INFO *io_info)
{
	...
	u32 *track_stream = &current_ld_SD->mruBitMap, streamNum, shiftedValues,unshiftedValues;
	...
	/* if we found a stream, update the raid context and also update the mruBitMap */
		if(current_SD->nextSeqLBA && /*	boundry condition */
		...
		/*
 		 ...
 		 */
			...
			unshiftedValues = *track_stream & ~(shiftedValuesMask| indexValueMask);
 	...
 	*/
	streamNum = (*track_stream >> ((MAX_STREAMS_TRACKED -1 ) *BITS_PER_INDEX_STREAM)) & 
					STREAM_MASK;
	current_SD = &current_ld_SD->streamTrack[streamNum];
	current_SD->isRead = io_info->isRead;
	current_SD->nextSeqLBA = io_info->ldStartBlock + io_info->numBlocks;
...
/**
 * megasas_set_raidflag_cpu_affinity - This function sets the controller cpu
 * 				affinity in the raid context based on IO type.
...
*/
static void
megasas_set_raidflag_cpu_affinity(RAID_CONTEXT_UNION *pRAID_Context, MR_LD_RAID *raid,
	bool fp_possible, u8 isRead, u32 scsi_buff_len)
{
	u8 cpuSel = MR_RAID_CTX_CPUSEL_0;

	if (fp_possible) {
		if (isRead) {
			if ((raid->cpuAffinity.pdRead.cpu0) && (raid->cpuAffinity.pdRead.cpu1))
				cpuSel = MR_RAID_CTX_CPUSEL_FCFS;
			else if (raid->cpuAffinity.pdRead.cpu1)
				cpuSel = MR_RAID_CTX_CPUSEL_1;
		} else {
			if ((raid->cpuAffinity.pdWrite.cpu0) && (raid->cpuAffinity.pdWrite.cpu1))
				cpuSel = MR_RAID_CTX_CPUSEL_FCFS;
			else if (raid->cpuAffinity.pdWrite.cpu1)
				cpuSel = MR_RAID_CTX_CPUSEL_1;
			/* This is fpCacheBypassCapable R0/R1 VD */
			if ((raid->level <= 1) && (raid->capability.fpCacheBypassCapable)) {
				pRAID_Context->raid_context_g35.routingFlags |=
		...
		if (isRead) {
			...
			if ((raid->cpuAffinity.ldWrite.cpu0) && (raid->cpuAffinity.ldWrite.cpu1))
				cpuSel = MR_RAID_CTX_CPUSEL_FCFS;
			else if (raid->cpuAffinity.ldWrite.cpu1)
				cpuSel = MR_RAID_CTX_CPUSEL_1;

			if (is_stream_detected(&pRAID_Context->raid_context_g35) &&
				((raid->level == 5) || (raid->level == 6)) &&
				(raid->writeMode == MR_RL_WRITE_THROUGH_MODE) &&
				(cpuSel == MR_RAID_CTX_CPUSEL_FCFS))
				cpuSel = MR_RAID_CTX_CPUSEL_0;
		}
	}

	pRAID_Context->raid_context_g35.routingFlags |=
		(cpuSel << MR_RAID_CTX_ROUTINGFLAGS_CPUSEL_SHIFT);

	/* Always give priority to MR_RAID_FLAGS_IO_SUB_TYPE_LDIO_BW_LIMIT
	* vs MR_RAID_FLAGS_IO_SUB_TYPE_CACHE_BYPASS.
	* IO Subtype is not bitmap.
	*/
	if ((raid->level == 1) && (!isRead)) {
			  ...
			  struct megasas_cmd_fusion *cmd)
{
	...
	io_request->RaidContext.raid_context.VirtualDiskTgtId = cpu_to_le16(device_id);
	...
	if (instance->adapter_type == VENTURA_SERIES) {
		 ...
		 *  All large RAID-1 writes (>= 64 KiB, both WT and WB modes)
		 * are built by the driver as LD I/Os.
		 * All small RAID-1 WT writes (< 64 KiB) are built as FP I/Os
		 * (there is never a reason to process these as buffered writes)
		 * All small RAID-1 WB writes (< 64 KiB) are built as FP I/Os
		...
		*/
		...
		if (raid)
			megasas_set_raidflag_cpu_affinity(pRAID_Context, raid, fp_possible,
							io_info.isRead, scsi_buff_len);
		else
			/* raid is NULL, set CPU affinity to default CPU0 */
			pRAID_Context->raid_context_g35.routingFlags |=
	...
	if (fp_possible) {
		...
		cmd->request_desc->SCSIIO.RequestFlags =
			(MPI2_REQ_DESCRIPT_FLAGS_FP_IO
			 << MEGASAS_REQ_DESCRIPT_FLAGS_TYPE_SHIFT);
		if (instance->adapter_type == INVADER_SERIES) {
			if (instance->divert_io_with_chain_frame &&
			    (cmd->sge_count > fusion->max_sge_in_main_msg)) {
				io_request->RaidContext.raid_context.VirtualDiskTgtId = cpu_to_le16(device_id);
				if (io_info.do_fp_rlbypass ||
				    (io_request->RaidContext.raid_context.regLockFlags == REGION_TYPE_UNUSED)) {
					cmd->request_desc->SCSIIO.RequestFlags =
				...
				io_request->RaidContext.raid_context.divertedFPIO = 1;
			} else {
				...
				io_request->RaidContext.raid_context.regLockFlags |= (MR_RL_FLAGS_GRANT_DESTINATION_CUDA | MR_RL_FLAGS_SEQ_NUM_ENABLE);
			}
			...
			io_request->RaidContext.raid_context_g35.nsegType |=
							(1 << RAID_CONTEXT_NSEG_SHIFT);
			io_request->RaidContext.raid_context_g35.nsegType |=
							(MPI2_TYPE_CUDA << RAID_CONTEXT_TYPE_SHIFT);
			io_request->RaidContext.raid_context_g35.routingFlags |=
		...
		if (fusion->load_balance_info &&
			...
			io_info.isRead) {
			io_info.devHandle = get_updated_dev_handle(instance,
						&fusion->load_balance_info[device_id], &io_info, local_map_ptr);
...
/* CS2.1 */	
		...
		if (instance->adapter_type == INVADER_SERIES) {
			...
			io_request->RaidContext.raid_context.Type = MPI2_TYPE_CUDA;
			io_request->RaidContext.raid_context.regLockFlags |= (MR_RL_FLAGS_GRANT_DESTINATION_CPU0 | MR_RL_FLAGS_SEQ_NUM_ENABLE);
			io_request->RaidContext.raid_context.nseg = 0x1;
		} else if (instance->adapter_type == VENTURA_SERIES) {
			io_request->RaidContext.raid_context_g35.routingFlags |=
				(1 << MR_RAID_CTX_ROUTINGFLAGS_SQN_SHIFT);
	...
	struct scsi_cmnd *scmd, struct megasas_cmd_fusion *cmd, u8 fp_possible)
{
	...
	cmd->pdInterface = mr_device_priv_data->interface_type;
	...
	if (instance->use_seqnum_jbod_fp &&
		...
		if(instance->support_morethan256jbod) /* More than 256 PD/JBOD support for Ventura */
	...
	if (!fp_possible) {
		...
		/* Hardware errata. Fast Path engine need one second extra to avoid early timeout.*/
		if (os_timeout_value)
			os_timeout_value++;
			...
			struct megasas_cmd_fusion *cmd)
{
	...
	megasas_make_sgl(instance, scp, cmd, sge_count);

	if (instance->adapter_type == VENTURA_SERIES) {
		set_num_sge(&io_request->RaidContext.raid_context_g35, sge_count);
		cpu_to_le16s(&io_request->RaidContext.raid_context_g35.routingFlags);
		cpu_to_le16s(&io_request->RaidContext.raid_context_g35.nsegType);
	} else {
...
{
	MEGASAS_REQUEST_DESCRIPTOR_UNION *req_desc, *req_desc2=NULL;
	struct fusion_context *fusion;
	fusion = instance->ctrl_context;
	req_desc = cmd->request_desc;
	/* copy the io request frame as well as 8 SGEs data for r1 command*/
	memcpy(r1_cmd->io_request, cmd->io_request,
			(sizeof(MEGASAS_RAID_SCSI_IO_REQUEST)));
	...
	atomic_inc(&instance->r1_fp_writes_count);
}
...
/**
 * megasas_complete_r1_command - Completes R1 FP Write commands which has valid peer smid
 * @instance:			Adapter soft state
 * @cmd_fusion:			MPT command frame
 *
 */
static inline void megasas_complete_r1_command(struct megasas_instance *instance,
		struct megasas_cmd_fusion *cmd)
{
	u8 *sense, status, extStatus;
	u32 data_length;
	struct fusion_context *fusion;
	...
	fusion = instance->ctrl_context;
	r1_cmd = fusion->cmd_list[le16_to_cpu(cmd->io_request->RaidContext.raid_context_g35.smid.peerSMID) - 1];
	scmd_local = cmd->scmd;
	status = cmd->io_request->RaidContext.raid_context.status;
	extStatus = cmd->io_request->RaidContext.raid_context.exStatus;
	...
	if (r1_cmd->cmd_completed) {
		...
		map_cmd_status(fusion, scmd_local, status, extStatus,
			...
			atomic_dec(&instance->fw_outstanding);
 			if ((cmd_fusion->r1_alt_dev_handle == MR_DEVHANDLE_INVALID)) {
 				map_cmd_status(fusion, scmd_local, status,
					extStatus, le32_to_cpu(data_length), sense);
 				if (instance->ldio_threshold && megasas_cmd_type(scmd_local) == READ_WRITE_LDIO)
 					atomic_dec(&instance->ldio_outstanding);
				scmd_local->SCp.ptr = NULL;
...
megasas_alloc_host_crash_buffer(struct megasas_instance *instance)
{
	...
	for (i = 0; i < MAX_CRASH_DUMP_SIZE; i++) {
	 	instance->crash_buf[i] = vmalloc(CRASH_DMA_BUF_SIZE);
		if (!instance->crash_buf[i]) {
			dev_err(&instance->pdev->dev, "crash dump mem alloc failed at index %d\n", i);
			break;
		} else
			memset(instance->crash_buf[i], 0, CRASH_DMA_BUF_SIZE);
	}
...
#if BLK_TAG_DEBUG
			dev_info(&instance->pdev->dev,
				"%s : action (%d) sync_cmd_idx (%04d) tag (%04d) cmd (0x%x) opcode (0x%x)\n",
				__func__, result, cmd_fusion->sync_cmd_idx,
				(smid - 1),
				(cmd_mfi->frame->hdr.cmd), opcode);
#endif
	...
	struct MR_PRIV_DEVICE *mr_device_priv_data)
{
	...
	MEGASAS_REQUEST_DESCRIPTOR_UNION *req_desc;
	struct fusion_context *fusion;
	struct megasas_cmd_fusion *scsi_lookup = NULL;
	int rc;
...
transition_to_ready:
			...
			if (instance->adapter_type == VENTURA_SERIES) {
				for(j=0; j< MAX_LOGICAL_DRIVES_EXT; ++j) {
					memset(fusion->streamDetectByLD[j], 0, sizeof(LD_STREAM_DETECT));
...
megasas_alloc_fusion_context(struct megasas_instance *instance)
{
	...
	if (!instance->ctrl_context) {
		...
		instance->ctrl_context = vmalloc(sizeof(struct fusion_context));
		if (!instance->ctrl_context) {
			dev_err(&instance->pdev->dev, "Failed from %s %d\n", __func__, __LINE__);
			return -ENOMEM;
		} else
			memset(instance->ctrl_context, 0, sizeof(struct fusion_context));
	}
	...
	fusion->load_balance_info = (PLD_LOAD_BALANCE_INFO) __get_free_pages(GFP_KERNEL | __GFP_ZERO,
		fusion->load_balance_info_pages);
	if (!fusion->load_balance_info) {
		fusion->load_balance_info = vmalloc(MAX_LOGICAL_DRIVES_EXT *
			sizeof(LD_LOAD_BALANCE_INFO));
		if (!fusion->load_balance_info)
			...
			memset(fusion->load_balance_info, 0,
				(MAX_LOGICAL_DRIVES_EXT * sizeof(LD_LOAD_BALANCE_INFO)));
	}
